...
public class OracleDatabaseFns extends DatabaseFns {
  ...
  private static final String partialSql =
                  ...
                  db -> {
                      ...
                      schemaTables.addAll(
                          tablesSplit.stream()
                              .filter(
                                  t ->
                                      t.startsWith(schema + ".")
                                          && !schemaTables.contains(t.split("\\.")[1]))
                              .map(t -> t.split("\\.")[1])
                              .collect(Collectors.toList()));
                    }
                    ...
                    try (ResultSet tables =
                      ...
                      return tablesList.stream()
                          .filter(o1 -> schemaTableList.stream().anyMatch(o2 -> o2.equals(o1)))
                          .collect(Collectors.toList());
                    }
                  });
            })
            ...
            () -> {
              ...
              return dbb.transactReturning(
                  db -> {
                    ...
                    if (job.tablesSplit.contains(
                        schema + "."
                            + tableName)) { // } || job.tablePriorities.contains(schema + "." +
                      // tableName)) {
                      conditionalQuery =
                          partialSql
                    ...
                    try {
                      ...
                      if (job.unionizeQuery.contains(schema + "." + tableName)) {
                        ...
                        batches.stream()
                            .forEach(
                                batch -> {
                                  String result =
                                      batch.stream().collect(Collectors.joining(" UNION \n"));
                                  unionizeQueries.add(result);
                                });
                        table.setQueryCount(unionizeQueries.size());
                        queries =
                            unionizeQueries.stream()
                                .collect(
                                    Collectors.mapping(
                                        query ->
                                            new Query(
                                                ...
                                                StringUtils.leftPad(
                                                    ...
                                                    7,
                                                    "0"),
                                                getId(query, startPattern)
                                                    .replace('/', '_')
                                                    .replace('+', '_'),
                                                getId(query, endPattern)
                                                    .replace('/', '_')
                                                    .replace('+', '_')),
                                        Collectors.toList()));
                      } else {
                      ...
                      if (!job.continueOnException)
                        throw new Exception(
                            String.format(
                                Locale.ROOT,
                                "getQueries - tableName %s. Exception: %s)",
                                tableName,
                                e));
                    }
                    return queries;
