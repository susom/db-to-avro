...
public class DatabaseProviderRx implements Supplier<Database> {
   ...
   * destroy the pool. You will most likely want to register a JVM shutdown hook to make sure this happens.</p>
   */
  @CheckReturnValue
  ...
  /**
   * Use an externally configured DataSource, Flavor, and optionally a shutdown hook. The shutdown hook may be null if
   * you don't want calls to Builder.close() to attempt any shutdown. The DataSource and Flavor are mandatory.
   */
  @CheckReturnValue
   ...
   * rollback if it throws an error). Note you will get a DatabaseException if you try to call this from the event loop
   * thread. If the provided code block throws anything, it will be wrapped into a DatabaseException unless it was a
   * ThreadDeath or was already a DatabaseException.
   */
  ...
  /**
   * Same as {@link #transact(DbCode)}, but your code block can explicitly manage the behavior of the transaction.
   */
  ...
  /**
   * Execute a transaction (no return value) with default semantics (commit if the code completes successfully, or
   * rollback if it throws an error).
   */
  ...
  /**
   * Same as {@link #transactRx(DbCode)}, but your code block can explicitly manage the behavior of the transaction.
   */
  ...
  /**
   * Execute a transaction, returning the result of the query, with default semantics (commit if the code completes
   * successfully, or rollback if it throws an error). Null return values signal OnComplete(), non-null signal
   * onSuccess()
   */
  public <T> Maybe<T> transactRx(final DbCodeTyped<T> code) {
...
//        }
  ...
  /**
   * This builder is immutable, so setting various options does not affect the previous instance. This is intended to
   * make it safe to pass builders around without risk someone will reconfigure it.
   */
  public interface Builder {
    ...
    /**
     * Include SQL in exception messages. This will also include parameters in the exception messages if SQL parameter
     * logging is enabled. This is handy for development, but be careful as this is an information disclosure risk,
    ...
    /**
     * Wherever argDateNowPerDb() is specified, use argDateNowPerApp() instead. This is useful for testing purposes as
     * you can use OptionsOverride to provide your own system clock that will be used for time travel.
     */
    @CheckReturnValue
    DatabaseProviderRx.Builder withDatePerAppOnly();

    /**
     * Allow provided Database instances to explicitly control transactions using the commitNow() and rollbackNow()
     * methods. Otherwise calling those methods would throw an exception.
     */
    @CheckReturnValue
    DatabaseProviderRx.Builder withTransactionControl();

    /**
     * This can be useful when testing code, as it can pretend to use transactions, while giving you control over
     * whether it actually commits or rolls back.
     */
    @CheckReturnValue
    DatabaseProviderRx.Builder withTransactionControlSilentlyIgnored();

    /**
     * Allow direct access to the underlying database connection. Normally this is not allowed, and is a bad idea, but
     * it can be helpful when migrating from legacy code that works with raw JDBC.
     */
    @CheckReturnValue
    DatabaseProviderRx.Builder withConnectionAccess();

    /**
     * WARNING: You should try to avoid using this method. If you use it more that once or twice in your entire codebase
    ...
    <T> Maybe<T> transactRx(DbCodeTyped<T> code);
  ...
  private static class BuilderImpl implements DatabaseProviderRx.Builder {
    ...
    private BuilderImpl(Closeable pool, Supplier<Connection> connectionProvider,
        Options options) {
      this.pool = pool;
      this.connectionProvider = connectionProvider;
      this.options = options;
    ...
    public DatabaseProviderRx.Builder withOptions(OptionsOverride options) {
      return new DatabaseProviderRx.BuilderImpl(pool, connectionProvider, options.withParent(this.options));
    }

    @Override
    public DatabaseProviderRx.Builder withSqlParameterLogging() {
      return new DatabaseProviderRx.BuilderImpl(pool, connectionProvider, new OptionsOverride() {
        @Override
